...
public class MeshSerializer
{
	...
	public static Mesh ReadMesh(byte[] bytes)
	{
		...
		if ((format & 16) != 0)
		{
			mesh.subMeshCount = buf.ReadUInt16();
			for (i = 0; i < mesh.subMeshCount; i++)
			{
				var subMeshTriCount = buf.ReadUInt16();
				var tris = new int[subMeshTriCount * 3];
				for (int j = 0; j < subMeshTriCount; j++)
				{
					tris[j * 3 + 0] = buf.ReadUInt16();
					tris[j * 3 + 1] = buf.ReadUInt16();
					tris[j * 3 + 2] = buf.ReadUInt16();
				}
	...
	static void ReadVector3Array16Bit(Vector3[] arr, BinaryReader buf)
	{
        ...
        float xscale = (bmax.x - bmin.x) / 65535.0f;
        float yscale = (bmax.y - bmin.y) / 65535.0f;
        float zscale = (bmax.z - bmin.z) / 65535.0f;
        ...
        for (var i = 0; i < n; ++i)
		{
			...
			float xx = ix * xscale + bmin.x;
			float yy = iy * yscale + bmin.y;
			float zz = iz * zscale + bmin.z;
	...
	public static byte[] WriteMesh(Mesh mesh, bool saveTangents)
	{
		...
		if ((format & 16) != 0)
		{
			var subMeshesCount = (ushort)mesh.subMeshCount;
			buf.Write(subMeshesCount);
			for (int i = 0; i < subMeshesCount; i++)
			{
				var subMeshTris = mesh.GetTriangles(i);
				var subMeshTriCount = (ushort)(subMeshTris.Length / 3);
				buf.Write(subMeshTriCount);
				foreach (var idx in subMeshTris)
				{
